...
def main():
    model_obj = GenerateData(type="blobs", n_samples=500, n_features=2, centers=3)
    ...
    #model_obj.draw()
    model_obj.setModel(KMeansClustering(iters=5))
    model_obj.train()
    model_obj.transform()
    ...
    # model_obj.loadData(lambda x: abs(0.6 * abs(1.4 * x - 8) - 1), 0, 10, 1000, 0.1)
    # model_obj.split(test_size=0.2)

    # model_obj.setModel(NeuralNetwork(learning_rate=0.05, n_iters=50000,
    #                                  hidden_nodes=[2, 3],
...
#             print(f"Centroids: {self.clusters}")
